...
#define RI_DATAExtSUPPORT2 0x102    // 支持扩展数据传输 通视扩展协议   V3.62新增　扩充数据以扩展方式提供（存放在买卖5数据后面8字节）行情大小变为166字节
...
//#define		TIME_TYPE				time_t		// UCT 适用VC6版本 新版本VS2005及以上版本 已变成8字节时间 可改为 __time32_t 类型 使用下一行代替
#define		TIME_TYPE				__time32_t	// 新版本VS2005及以上版本 已变成8字节时间 如果使用新版本VC请注释上一行 启用这一行
...
struct tagMy_PankouType
{
	WORD	m_wMarket;             // 股票市场类型　　　　　　　　同通视规范定义       2字节
	WORD	m_wStkIdx;             // 股票在该市场的码表索引      V3.62 版本修改字段   2字节
	char	m_szLabel[STKLABEL_LEN];     // 股票代码,以'\0'结尾,如 "600100"  同通视规范定义  10字节
	long	m_lDate;            // FORMAT: 20010305,0表示当天或最近交易日的明细数据 4字节
	float	m_fLastClose;        // 昨收                                             4字节 
	float	m_fOpen;             // 今开                                             4字节 
	DWORD	m_nAllCount;          // 总盘口行情数                                     4字节
	DWORD	m_nCount;             // 当前块笔数        V3.62 版本从双字节扩充到       4字节 m_nCount<=m_nAllCount 一个品种的分笔可能通过多个包传送 如果分笔数据量很大的话 需要应用层自己合并
	WORD	R0;                    // 保留字段          V3.62 版本                     2字节 数据头部一共 36 字节 后面数据体长度为 116*m_nCount
	tagRCV_PANKOU_STRUCTEx	m_Data[1]; //    V3.62 版本每笔扩充8字节 该缓冲区的长度为 m_nCount*sizeof(RCV_PANKOU_STRUCTEx)=m_nCount*116 该缓冲区的每笔定义见上述 RCV_PANKOU_STRUCTEx
