...
int main(int argc, char **argv)
{
	...
	int cout_log_level = utility::Applog::LOG_ERROR;
#ifdef _DEBUG
#else
...
#endif
	cout_log_level = utility::Applog::LOG_INFO;
	utility::Applog::SetLogLevel(cout_log_level);
	...
	delete[] tmplmt;
#endif
	CProjectServer::create_instance();
	CProjectServer::get_instance()->startServer();
	while(!b_stop)
	{
		//cout << "master thread" << endl;
		q_sleep(1000);
	}
	CProjectServer::get_instance()->stopServer();   
	CProjectServer::get_instance()->Join();
    CProjectServer::destory_instance();
	return 0;
}
